...
// Função para gerar QR Code para um checklist
export async function generateQRCode(checklistId) {
  try {
    ...
    const qrFilePath = path.join(qrDir, `checklist-${checklistId}.png`);
    
    // URL para acessar o checklist (ajuste conforme sua aplicação)
    const checklistUrl = `/professional/execute-checklist/${checklistId}`;
    
    // Gerar QR Code
    await QRCode.toFile(qrFilePath, checklistUrl, {
      color: {
        dark: '#000',
        light: '#fff'
      },
    ...
    // Retornar o caminho relativo para o QR Code
    return `/qrcodes/checklist-${checklistId}.png`;
  } catch (error) {
